...
const HomeLinksPage = () => {
    ...
    const {mock,idMock,mockData}=React.useContext(AuthContext)
...
console.log(idMock,mock,mockData)
  const goToLink = (data) => {
    history.push(`/home/${data}`);
  };
// const itm=mockData?.filter((item)=>item.id===idMock)
// console.log(itm)
  return (
    <>
      <Navbar />
      {mock?mockData?.filter((item)=>parseInt(item.id)===parseInt(idMock)).map((item)=><NewsWrapper>
      <h1>{item.title}</h1>
      <h3>By {item.author}, CNN</h3>
      <p> {item.time}</p>
      <div>
        <img src={item.img} alt="mock pic" />
      </div>
      <br></br>
      <HealthStory>
              {/* <p>
